...
FROM TB_STUDENT
--WHERE ENTRANCE_DATE LIKE '9_%' -- 학생 번호였나봄
-- 주소지가 강원도나 경기도인가를 조회하는 쿼리문 빠짐
-- 아래는 고친거
...
WHERE CATEGORY = '인문사회' -- 인문 사회 중간에 띄어쓰기 때문에 결과가 안나왔던거;
ORDER BY 2, 1
;
...
-- 그룹이랑 테이블이랑 거는 조건은 다르다
-- WHERE절은 하나하나 검사하기 때문에 AVG 평균값 구하는 쿼리문을 작성할 수 x
HAVING AVG(POINT) >= 4
...
-- 16번
-- 환경조경학과 전공과목들의 "과목 별 평점"을 파악할 수 있는 SQL 문을 작성하시오.
SELECT 
    D.DEPARTMENT_NAME AS 과목명 --> 이건 과목명이 아니라 학과명!
...
SELECT
    CLASS_NO
    , CLASS_NAME AS 과목명 --> 이건 과목명이 아니라 학과명!
    , AVG(POINT) AS 평균평점
FROM TB_CLASS 
JOIN TB_GRADE G USING(CLASS_NO) --ON C.CLASS_NO = G.CLASS_NO
JOIN TB_DEPARTMENT D USING(DEPARTMENT_NO) --ON C.DEPARTMENT_NO = D.DEPARTMENT_NO
WHERE D.DEPARTMENT_NAME = '환경조경학과'
...
-- 18번
-- 국어국문학과에서 "총점수"가 가장 높은 학생의 이름과 학번을 표시하는 SQL문을 작성하시오
