...
-- 3번
-- 주소지가 강원도나 경기도인 학생들 중 1900년대 학번을 가진 학생들의 이름과 학번, 주소를 이름의 오름차순으로 화면에 출력하시오. -- 여기 있는 학번은 학생 번호
-- 단, 출력헤더에는 "학생이름", "학번", "거주지 주소"가 출력되도록 한다. -- 여기서의 학번은 입학년도. 예) 00학번
SELECT STUDENT_NAME "학생 이름"
    , STUDENT_NO "학번 (학생번호)" -- 일단 헷갈리니까 얘 냅두고
    , SUBSTR(ENTRANCE_DATE, 1, 2) "학번 (입학년도)"
    , STUDENT_ADDRESS "거주지 주소"
FROM TB_STUDENT
WHERE ENTRANCE_DATE LIKE '9_%'
;
...
SELECT 
    D.DEPARTMENT_NAME AS 과목명
    , ROUND(AVG(G.POINT), 1) AS 평균평점
FROM TB_GRADE G
JOIN TB_CLASS C ON G.CLASS_NO = C.CLASS_NO
JOIN TB_STUDENT S ON G.STUDENT_NO = S.STUDENT_NO
JOIN TB_DEPARTMENT D ON S.DEPARTMENT_NO = D.DEPARTMENT_NO
...
SELECT S.STUDENT_NO, S.STUDENT_NAME, SUM(G.POINT) AS TOTAL_POINT
FROM TB_GRADE G
JOIN TB_CLASS C ON G.CLASS_NO = C.CLASS_NO
JOIN TB_DEPARTMENT D ON C.DEPARTMENT_NO = D.DEPARTMENT_NO
JOIN TB_STUDENT S ON G.STUDENT_NO = S.STUDENT_NO
...
HAVING SUM(G.POINT) = (
    SELECT MAX(TOTAL)
    FROM (
        SELECT G.STUDENT_NO, SUM(G.POINT) AS TOTAL
        FROM TB_GRADE G
        JOIN TB_CLASS C ON G.CLASS_NO = C.CLASS_NO
        JOIN TB_DEPARTMENT D ON C.DEPARTMENT_NO = D.DEPARTMENT_NO
        WHERE D.DEPARTMENT_NAME = '국어국문학과'
...
--WHERE C.CLASS_NAME = '국어국문학과'
----국어국문학과에서 총점수가 가장 높은 학생
--;
--
...
GROUP BY D.DEPARTMENT_NAME --, C.CLASS_TYPE
ORDER BY D.DEPARTMENT_NAME
;
