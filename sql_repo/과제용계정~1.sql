SELECT ANIMAL_ID, NAME
FROM ANIMAL_INS
ORDER BY ANIMAL_ID;

-- 나이 정보가 없는 회원 수 구하기

SELECT USER_ID(*) AS USERS
FROM USER_INFO
WHERE AGE IS NULL
;

-- 강원도에 위치한 생산공장 목록 출력하기

SELECT FACTORY_ID
, FACTORY_NAME
, ADDRESS
FROM FOOD_FACTORY
ORDER BY FACTORY_ID ASC;

-- 조건에 맞는 회원수 구하기

SELECT
    COUNT(*) AS USERS
FROM USER_INFO
WHERE AGE BETWEEN 20 AND 29
AND JOINED LIKE 2021
;

-- 흉부외과 또는 일반외과 의사 목록 출력

SELECT DR_NAME, DR_ID, MCDP_CD, TO_CHAR(HIRE_YMD, 'YYYY-MM-DD') as HIRE_YMD
FROM DOCTOR
WHERE MCDEP_CD = 'CS' OR MCDP_CD='GS'
ORDER BY HIRE_YMD DESC, DR_NAME;
-- 고용일자가 같다면 이름을 기준으로 오름차순 정렬


-- 어린 동물 찾기
SELECT ANIMAL_ID, NAME, INTAKE_CONDITION
FROM ANIMAL_INS
WHERE INTAKE_CONDITION <> 'Aged'
ORDER BY ANIMAL_ID
;

-- 역순 정렬하기
SELECT NAME, DATETIME
FROM ANIMAL_INS
ORDER BY ANIMAL_ID DESC
;

-- 상위 n개 레코드
SELECT NAME
FROM ANIMAL_INS
WHERE DATETIME
; -- 상위를 가리는? sql문은 배운적 없으니 보류

-- 여러 기준으로 정렬하기
SELECT ANIMAL_ID
       , NAME
       , DATETIME
FROM ANIMAL_INS
ORDER BY NAME, DATETIME DESC --이름이 같은 동물 중에서는 보호를 나중에 시작한 동물
;

-- 이름이 있는 동물의 아이디
SELECT ANIMAL_ID
FROM ANIMAL_INS
HAVING NAME IS NOT NULL -- 이름이 있는 동물의 ID 
ORDER BY ANIMAL_ID ASC
;


-- 12세 이하인 "여자 환자" 목록 출력
 SELECT PT_NAME, PT_NO, GEND_CD, AGE, NVL(TLNO, NONE)-- 전화번호가 없는 경우 NONE으로 출력할 것
 FROM PATIENT
 WHERE AGE <= 12
 AND GEND_CD = 'W' --나이가 같다면 환자이름 순
 ORDER BY AGE DESC, PT_NAME ASC 
 ;
 
-- 나이가 같다면 환자이름 순.. 이걸 어떻게 작성해야..?
--> 얜 상관 x



-- 인기있는 아이스크림

SELECT FLAVOR
FROM FIRST_HALF
ORDER BY TOTAL_ORDER DESC, SHIPMENT_ID ASC
;

-- 조건에 맞는 도서 리스트 출력
SELECT BOOK_ID, TO_CHAR(PUBLISHED_DATE, 'YYYY-MM-DD') as PUBLISHED_DATE
FROM BOOK
WHERE CATEGORY = '인문'
    AND TO_CHAR(PUBLISHED_DATE, 'YYYY') = '2021'
--    AND PUBLISHED_DATE LIKE '2021%'
ORDER BY PUBLISHED_DATE ASC
;


-- 과일로 만든 아이스크림 고르기

SELECT A.FLAVOR
FROM FIRST_HALF A
JOIN ICECREAM_INFO B ON A.FLAVOR = B.FLAVOR
WHERE A.TOTAL_ORDER >= 3000
AND B.INGREDIENT_TYPE = 'fruit_based'
ORDER BY A.TOTAL_ORDER
;


-- 평균 일일 대여 요금 구하기

SELECT ROUND(AVG(DAILY_FEE)) --ROUND(AVG(DAILY_FEE)) 평균 일일 대여 요금 (소수 첫 번째 자리 반올림) AS AVERAGE_FEE
FROM CAR_RENTAL_COMPANY_CAR
WHERE CART_TYPE = 'SUV'
;

-- 모든 레코드 조회하기
SELECT *
FROM ANIMAL_INS
ORDER BY ANIMAL_ID
;


-- 특정 옵션이 포함된 자동차
SELECT CAR_ID, CAR_TYPE, DAILY_FEE, OPTIONS
FROM CAR_RENTAL_COMPANY_CAR
WHERE OPTIONS LIKE '%네비게이션%'
ORDER BY CAR_ID DESC
;

-- 최댓값 구하기
SELECT MAX(DATETIME)
FROM ANIMAL_INS
;

--자동차 대여 기록에서 장기/단기 대여 구분하기
--대여 시작일이 2022년 9월에 속하는 대여 기록에 대해서 대여 기간이 30일 이상이면 '장기 대여' 그렇지 않으면 '단기 대여' 로 표시하는 컬럼(컬럼명: RENT_TYPE)
--SELECT HISTORY_ID
--    , CARD_ID
--    , START_DATE
--    , END_DATE
--    , RENT_TYPE
--    , CASE
--    WHEN MONTHS_BETWEEN(START_DATE, END_DATE) >= 30 THEN '장기 대여' --대여기간 30일 이상 --(SUBSTR(START_DATE, 9, 2) - SUBSTR(END_DATE, 9, 2)) = 30
--    ELSE '단기 대여'
--    END AS RENT_TYPE
--FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
--ORDER BY HISTORY_ID DESC
--;

SELECT HISTORY_ID
    , CAR_ID
    , START_DATE
    , END_DATE
    , CASE
    WHEN TO_CHAR((MONTHS_BETWEEN(START_DATE, END_DATE)), 'YYYY-MM-DD') >= 30 THEN '장기 대여'
    ELSE '단기 대여'
    END AS RENT_TYPE
FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
ORDER BY HISTORY_ID DESC
;


SELECT HISTORY_ID
    , CAR_ID
    , TO_CHAR(START_DATE, 'YYYY-MM-DD') AS START_DATE
    , TO_CHAR(END_DATE, 'YYYY-MM-DD') AS END_DATE
    , CASE
    WHEN END_DATE - START_DATE + 1 >= 30 THEN '장기 대여' --> 왜 +1을 하는거지?
    ELSE '단기 대여'
    END AS RENT_TYPE
FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
WHERE EXTRACT(MONTH FROM START_DATE) = 9
ORDER BY HISTORY_ID DESC
;

-- 조건에 부합하는 중고거래 댓글 조회
SELECT * --  게시글 제목, 게시글 ID, 댓글 ID, 댓글 작성자 ID, 댓글 내용, 댓글 작성일
FROM USED_GOODS_BOARDS GB
JOIN USED_GOODS_REPLY GR ON GB.CREATED_DATE = GR.CREATED_DATE
WHERE CREATED_DATE LIKE '2022-10___'
ORDER BY CREATED_DATE ASC --결과는 댓글 작성일을 기준으로 오름차순 정렬
, TITLE--"댓글 작성일이 같다"면 게시글 제목을 기준
;


SELECT * --  게시글 제목, 게시글 ID, 댓글 ID, 댓글 작성자 ID, 댓글 내용, 댓글 작성일
FROM USED_GOODS_BOARD --GB
--JOIN USED_GOODS_REPLY GR ON GB.BOARD_ID = GR.BOARD_ID
WHERE CREATED_DATE = CREATED_DATE --GB.CREATED_DATE = GR.CREATED_DATE
ORDER BY CREATED_DATE ASC --결과는 댓글 작성일을 기준으로 오름차순 정렬
, TITLE--"댓글 작성일이 같다"면 게시글 제목을 기준
;


SELECT *
FROM USED_GOODS_BOARD
WHERE CREATED_DATE = CREATED_DATE -- 테이블에서 2022년 10월에 작성된 게시글
AND 	CREATED_DATE LIKE '2022-10___'
ORDER BY CREATED_DATE ASC
, TITLE
;


SELECT B.TITLE
    , B.BOARD_ID
    , R.REPLY_ID
    , R.WRITER_ID
    , R.CONTENTS
    , TO_CHAR(R.CREATED_DATE, 'YYYY-MM-DD') CREATED_DATE
FROM USED_GOODS_BOARD B
JOIN USED_GOODS_REPLY R ON B.BOARD_ID = R.BOARD_ID
WHERE TO_CHAR(B.CREATED_DATE, 'YYYY-MM-DD') LIKE '2022-10-__'
--AND B.CREATED_DATE = R.CREATED_DATE
--ORDER BY TO_CHAR(CREATED_DATE, 'YYYY-MM-DD') ASC
ORDER BY CREATED_DATE ASC
, TITLE ASC
;